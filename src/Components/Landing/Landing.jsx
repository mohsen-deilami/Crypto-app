...
export default function Landing() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(
        `${BASE_URL}/markets?vs_currency=${currency}&per_page=100&order=market_cap_desc&${API_KEY}`,{
  ...
  const searchHandler = (searchText) => {
    if (searchText) {
      const coinData = searchData.filter((coin) =>
  ...
  return (
    <>
      <SearchBar
        selectCurrencyHandler={selectCurrencyHandler}
